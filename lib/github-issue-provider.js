import { r as validateAuthority, t as authorityFailure } from "./authority-Cq3uspKj.js";
//#region src/github-issue-provider.ts
/**
...
const GITHUB_ISSUE_KIND = "github-issue";
...
const GITHUB_USER_AGENT = "dsh-governed-workflow";
...
*/
const ALLOWED_BLOCK_KEYS = /* @__PURE__ */ new Set([
	"baselineRef",
	"baselineSha",
	"candidateBranch",
	"allowedPaths",
...
function stripFencedCodeBlocks(markdown) {
	...
	for (const line of lines) {
		...
		} else if (fence !== null && fence.char === fenceChar && fence.len >= fenceLen && fence.restIsBlank) inFence = false;
	}
	return out.join("\n");
}
/** Narrow `authorityFailure()` (always the failure variant) to the parse result's failure arm. */
function blockFailure(code, message, field) {
	return authorityFailure(code, message, field);
}
/**
...
*/
function parseV1AuthorityBlock(body) {
	const stripped = stripFencedCodeBlocks(body);
	const markers = [...stripped.matchAll(AUTHORITY_MARKER_RE)];
	if (markers.length === 0) return blockFailure("AUTHORITY_UNAVAILABLE", "no V1 authority block found in issue body");
	for (const marker of markers) if (marker[1] !== "v1") return blockFailure("INVALID_AUTHORITY", `unsupported authority block version "${marker[1]}"`);
	if (markers.length > 1) return blockFailure("INVALID_AUTHORITY", "multiple authority blocks found in issue body");
	const marker = markers[0];
	const openEnd = (marker.index ?? 0) + marker[0].length;
	const closeIndex = stripped.indexOf(MARKER_CLOSE, openEnd);
	if (closeIndex === -1) return blockFailure("INVALID_AUTHORITY", "unterminated authority block in issue body");
	const raw = stripped.slice(openEnd, closeIndex).trim();
	if (raw.length > 65536) return blockFailure("INVALID_AUTHORITY", "authority block is too large");
	let parsed;
	try {
	...
	if (!isPlainRecord(parsed)) return blockFailure("INVALID_AUTHORITY", "authority block must be a JSON object");
	for (const key of Object.keys(parsed)) if (!ALLOWED_BLOCK_KEYS.has(key)) return blockFailure("INVALID_AUTHORITY", `unexpected authority block field "${key}"`, key);
	return {
		ok: true,
...
*/
function validateGitHubIssueConfig(config) {
	const repository = config.repository;
	if (typeof repository !== "string") return "repository must be a string";
	if (repository.trim() !== repository) return "repository must not have surrounding whitespace";
	const segments = repository.split("/");
	if (segments.length !== 2 || !REPO_SEGMENT_RE.test(segments[0]) || !REPO_SEGMENT_RE.test(segments[1])) return "repository must be exactly \"OWNER/REPO\" with two nonblank path segments";
	const issueNumber = config.issueNumber;
	if (typeof issueNumber !== "number" || !Number.isSafeInteger(issueNumber) || issueNumber < 1) return "issueNumber must be a positive safe integer";
	return null;
}
/** Build the fixed public endpoint with URL-encoded path segments. */
function buildGitHubIssueUrl(repository, issueNumber) {
...
*/
var GitHubIssueAuthorityProvider = class {
	config;
	transport;
	...
	*/
	constructor(config, transport = defaultTransport) {
		this.config = config;
		this.transport = transport;
	}
	async resolve(options) {
		const signal = options?.signal;
		if (isAborted(signal)) return authorityFailure("INVALID_AUTHORITY", "authority request was aborted before it began");
		const configError = validateGitHubIssueConfig(this.config);
		if (configError !== null) return authorityFailure("INVALID_AUTHORITY", configError);
		...
		try {
			...
			if (isAborted(signal)) return authorityFailure("INVALID_AUTHORITY", "authority request was aborted");
			return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub request failed");
		}
		const status = response.status;
		if (status >= 300 && status < 400) return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub request was redirected; refusing to follow");
		if (status === 404 || status === 410) return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub issue is not available");
		if (status === 403 || status === 429) return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub request was rate-limited or forbidden");
		if (status !== 200) return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub request failed");
		let text;
		try {
		...
		if (text.length > 1048576) return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub response was too large");
		let payload;
		try {
		...
		if (!isPlainRecord(payload)) return authorityFailure("INVALID_AUTHORITY", "GitHub response was not a JSON object");
		if (payload.number !== this.config.issueNumber) return authorityFailure("INVALID_AUTHORITY", "GitHub response issue number did not match the configured target");
		if (payload.state !== "open") return authorityFailure("AUTHORITY_UNAVAILABLE", "GitHub issue is closed");
		if (Object.hasOwn(payload, "pull_request")) return authorityFailure("INVALID_AUTHORITY", "GitHub response was a pull request, not an issue");
		if (typeof payload.body !== "string") return authorityFailure("INVALID_AUTHORITY", "GitHub issue body was not a string");
...
//#endregion
export { GITHUB_ACCEPT, GITHUB_API_ORIGIN, GITHUB_API_VERSION, GITHUB_ISSUE_KIND, GITHUB_USER_AGENT, GitHubIssueAuthorityProvider, GitHubIssueAuthorityProvider as default, MAX_BLOCK_BYTES, MAX_RESPONSE_BYTES, buildGitHubIssueUrl, parseV1AuthorityBlock, validateGitHubIssueConfig };
