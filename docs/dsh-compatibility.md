...
The load smoke tests (V0 and V0.1) ran against the local `0.1.0-rc.5` checkout
at the revision above, via `pnpm dsh` (source execution). Compatibility is
pinned to that revision, not to a moving `main`.
...
- **DSH is in developer preview** and states there will be
  compatibility-breaking changes. The bundle manifest, patch syntax, and the
  `Service` base class are the surfaces most likely to shift; re-verify before
  each release.
- **Version skew.** The npm latest (`@deepseek-ai/dsh@0.1.0-rc.6`) is newer than
  the tested checkout (`0.1.0-rc.5`). Re-checked during V0.1; the plugin is
  still verified against the checkout revision and has **not** been re-verified
  against the published CLI. Compatibility claims are not silently broadened.
...
- **Types are authored but not shipped yet.** The `governance` service and its
  `ctx.governance` declaration merging are fully typed in source (and typecheck
  passes), but the package still ships JS only, so external consumers cannot yet
  import the `ctx.governance` type augmentation. Types are reintroduced as a
  `.d.ts` when a later task wires provider/guard/evidence packages that must
